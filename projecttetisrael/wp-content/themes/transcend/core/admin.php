...
function cpotheme_admin_welcome_notice(){	
	if(current_user_can('manage_options')){
		...
		if(current_user_can('manage_options') && $welcome_dismissed != 'dismissed' && $display){
			$welcome_url = '<a href="'.esc_url(admin_url('themes.php?page=cpotheme-welcome')).'">'.__('quickstart guide', 'transcend').'</a>';
			...
			echo '<a href="'.add_query_arg('ctdismiss', CPOTHEME_ID.'_wizard').'" class="cpothemes-notice-dismiss">'.__('Dismiss This Notice', 'transcend').'</a>';
			echo '<p>'.sprintf(esc_html__('%s is ready! Make sure you install the %s companion plugin and then check the quickstart guide to see how it all works.', 'transcend'), esc_attr(CPOTHEME_NAME), $plugin_url, $welcome_url).'</p>';
			echo '<p><a href="'.esc_url(admin_url('themes.php?page=cpotheme-welcome')).'" class="button button-primary" style="text-decoration: none;">'.sprintf(__('Start Using %s', 'transcend'), esc_attr(CPOTHEME_NAME)).'</a></p>';
			echo '</div>';
			echo '</div>';
		}
	}
